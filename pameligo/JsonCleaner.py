...
def extract_max_market_cap(filename):
    # Extract the number at the end of the filename before .json
    match = re.search(r'_(\d+)\.json$', filename)
    if match:
        return int(match.group(1))
    return 69000  # Default fallback if no number found
...
def clean_transaction_data(input_file, 
    ...
    # Get max_market_cap from filename
    max_market_cap = extract_max_market_cap(input_file)
